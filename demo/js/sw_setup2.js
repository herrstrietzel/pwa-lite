        ...
        switch (nodeName) {
            ...
            case 'link':
                ...
                if (rel === 'stylesheet' || src.endsWith('.css') || src.includes('googleapis.com/css2?')) {
                    if(!urlSet.has(src)){
                        assets.css.push({ src, cachePolicy });
                    }
                    urlSet.add(src)
                }
                ...
                // favicon/app icon
                    ...
                    if(!urlSet.has(src)){
                        assets.img.push({ src, cachePolicy, type:'favicon' });
                    }
                    urlSet.add(src)
                }
                ...
                // manifest
                    ...
                    // add manifest file
                    if(!urlSet.has(src)){
                        assets.misc.push({ src, cachePolicy, type:'manifest' });
                    }
                    urlSet.add(src)
                    ...
                    if (res.ok) {
                        ...
                        icons.forEach(icon => {
                            let icnSrc = icon.src || ''
                            if (icnSrc) {
                                //setup[cachePolicy].add(getAbsoluteUrl(icnSrc, src));
                                //assets.img.add({ src: getAbsoluteUrl(icnSrc, src), cachePolicy });
                                icnSrc = getAbsoluteUrl(icnSrc, src);

                                if(!urlSet.has(icnSrc)){
                                    assets.img.push({ icnSrc, cachePolicy, type:'appIcon' });
                                }
                                urlSet.add(icnSrc)
            ...
            case 'img':
                ...
                for (let prop in props) {
                    ...
                    imgSrcArr.forEach(imgSrc => {
                        //assets.img.add({ src: getAbsoluteUrl(imgSrc, src), cachePolicy });
                        imgSrc = getAbsoluteUrl(imgSrc, src);

                        if(!urlSet.has(imgSrc)){
                            assets.img.push({ imgSrc, cachePolicy, type:'img' });
                        }
                        urlSet.add(imgSrc)
            ...
            case 'script':
                ...
                //assets.js.add({ src, cachePolicy });

                if(!urlSet.has(src)){
                    assets.js.push({ src, cachePolicy, type:'js' });
                }
                urlSet.add(src)
        ...
        if (sheet.href) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    for (let i = 0, l = imports.length; l && i < l; i++) {
                        ...
                        //importUrl = importUrl ? getAbsoluteUrl(importUrl, href) : '';

                        // add import url
                        //setup[cachePolicy].add(importUrl);
                        //assets.css.add({ src: importUrl, cachePolicy })

                        if(!urlSet.has(importUrl)){
                            assets.css.push({ importUrl, cachePolicy, type:'cssImport' });
                        }
                        urlSet.add(importUrl)
            ...
            for (let i = 0, len = sheetInline.cssRules.length; len && i < len; i++) {
                ...
                if (rule.type === 5) {
                    ...
                    //assets.fonts.add({ src: getAbsoluteUrl(fontUrl, src), cachePolicy })

                    if(!urlSet.has(fontUrl)){
                        assets.fonts.push({ fontUrl, cachePolicy, type:'font' });
                    }
                    urlSet.add(fontUrl)
                ...
                // add background images
                    ...
                    if (backgroundImage) {
                        let imgSrc = getAbsoluteUrl(backgroundImage.split(/[\(|\)]/)[1]
                            .replace(/["|']/g, ''), href);
                        //setup.cache.add(src);
                        //assets.img.add({ src: getAbsoluteUrl(imgSrc, href), cachePolicy })

                        if(!urlSet.has(imgSrc)){
                            assets.img.push({ imgSrc, cachePolicy, type:'imgBackground' });
                        }
                        urlSet.add(imgSrc)
